    ...
    if ($id > 0) {
        ...
        if ($productos != null) {
            foreach ($productos as $clave => $cantidad) {
                ...
                FROM products WHERE id=? AND activo=1");
                $sql->execute([$clave]);
                $row_prod = $sql->fetch(PDO::FETCH_ASSOC); //lo va a asociar mediante el nombre de columna

                $precio = $row_prod['precio'];
                $descuento = $row_prod['descuento'];
                $precio_desc = $precio - (($precio * $descuento) / 100);

                $sql_insert = $con->prepare("INSERT INTO detalle_compra (id_compra,id_producto,
